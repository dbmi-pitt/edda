 ...
 */
public class EddaBinaryClassificationPerformance extends MeasuredPerformance {
	...
	public EddaBinaryClassificationPerformance() {
		type = -1;
	}

	public EddaBinaryClassificationPerformance(EddaBinaryClassificationPerformance o) {
		super(o);
		this.type = o.type;
	...
	public EddaBinaryClassificationPerformance(int type) {
		this.type = type;
	}

	/** For test cases only. */
	public EddaBinaryClassificationPerformance(int type, double[][] counter) {
		this.type = type;
	...
	public static EddaBinaryClassificationPerformance newInstance(String name) {
		for (int i = 0; i < NAMES.length; i++) {
			if (NAMES[i].equals(name))
				return new EddaBinaryClassificationPerformance(i);
		}
		return null;
	}
	...
	public double getMikroAverage() {
		...
		case PSEP:
			...
			break;
		case F3:
			Double precisionNumerator = counter[P][P];
			Double precisionDenominator = counter[P][P] + counter[N][P];
			Double recallNumerator = counter[P][P];
			Double recallDenominator = counter[P][P] + counter[P][N];
			x = 0.0d;
			y = 0.0d;
			if (precisionDenominator > 0.0d && recallDenominator > 0.0d) {
				Double precision = precisionNumerator / precisionDenominator;
				Double recall = recallNumerator / recallDenominator;
				x = 10.0d * (precision * recall);
				y = (9.0d * precision) + recall;
			}
			break;
		default:
...
//			return Double.NaN;
	...
	public void buildSingleAverage(Averagable performance) {
		EddaBinaryClassificationPerformance other = (EddaBinaryClassificationPerformance) performance;
