...
public class DocumentClusterExplorer {
	...
	public List<File> getCorpusFiles(){
		...
		for(String cluster: getClusterMap().keySet()){
			for(String f: getClusterMap().get(cluster)){
	...
	public void writeClusterSentences(File outputDir) throws Exception {
		HTMLClusterSummary html = new HTMLClusterSummary(this);
		...
		for(String cluster: getClusterMap().keySet()){
			...
			for(String document: getClusterMap().get(cluster)){
				...
				dw.write(ff.getName()+"\n");
				String path = ff.getParentFile().getName()+"/view.html?doc="+ff.getName();
			...
			// write html wrappers
			html.writeHTMLWrapper(f);
			html.writeHTMLWrapper(new File(outputDir,name+"-info.txt"));
			html.writeHTMLWrapper(new File(outputDir,name+"-summary.lexrank.html"));
			html.writeHTMLWrapper(new File(outputDir,name+"-summary.luhn.html"));
			html.writeHTMLWrapper(new File(outputDir,name+"-summary.tebeka.html"));
			html.writeHTMLWrapper(new File(outputDir,name+"-summary.random.html"));
		...
		for(File f: files){
			...
			String path = f.getParentFile().getName()+"/view.html?doc="+f.getName();
			docs.write("<a href=\""+path+"\">"+f.getName()+"</a>\n");
			...
			// viewer file
			File viewer = new File(f.getParentFile(),"view.html");
			if(!viewer.exists()){
				html.writeHTMLView(viewer);
			}
		}
		...
		// write html wrappers
		html.writeHTMLWrapper(new File(outputDir,"Corpus-sentences.txt"));
		html.writeHTMLWrapper(new File(outputDir,"Corpus-info.txt"));
		html.writeHTMLWrapper(new File(outputDir,"Corpus-summary.lexrank.html"));
